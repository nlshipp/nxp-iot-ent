    ...
    )
{
    ...
    if (!IMXUartIsRxDmaActive(interruptContextPtr) &&
        ...
        while (nextHead != tail) {
            ...
            if (interruptContextPtr->CurrentFlowReplace & SERIAL_AUTO_TRANSMIT) {
                if (static_cast<UCHAR>(rxd) == SERIAL_DEF_XOFF) {
                    interruptContextPtr->TxHoldState = IMX_UART_STATE::PAUSED;
                    IMX_UART_LOG_TRACE("XOFF detected.");
                    continue;
                } else {
                    if ((static_cast<UCHAR>(rxd) == SERIAL_DEF_XON) &&
                        (interruptContextPtr->TxHoldState == IMX_UART_STATE::PAUSED)) {
                        interruptContextPtr->TxHoldState = IMX_UART_STATE::STOPPED;
                        IMX_UART_LOG_TRACE("XON detected.");
                        continue;
                    }
                }
            }
        ...
        // Check if Xon/Xoff is enabled at receiver
        if (interruptContextPtr->CurrentFlowReplace & SERIAL_AUTO_RECEIVE) {
            // Check the Rx intermediate buffer items count.
            if (interruptContextPtr->RxHoldState == IMX_UART_STATE::STOPPED) {
                if ((interruptContextPtr->RxBuffer.Capacity() - interruptContextPtr->XoffThreshold) <=
                    interruptContextPtr->RxBuffer.Count()) {
                    // Xoff will be sent
                    interruptContextPtr->RxHoldState = IMX_UART_STATE::PAUSING;
                    IMX_UART_LOG_TRACE("XOFF will be sent");
                }
            } else {
                if (interruptContextPtr->RxHoldState == IMX_UART_STATE::PAUSED) {
                    if (interruptContextPtr->RxBuffer.Count() <= interruptContextPtr->XonThreshold) {
                        // Xon will be sent
                        interruptContextPtr->RxHoldState = IMX_UART_STATE::RESUMING;
                        IMX_UART_LOG_TRACE("XON will be sent");
                    }
                }
            }
        }
    ...
    if (!IMXUartIsTxDmaActive(interruptContextPtr)) {
       if (interruptContextPtr->CurrentFlowReplace & SERIAL_AUTO_RECEIVE) {
           if ((READ_REGISTER_NOFENCE_ULONG(&registersPtr->Uts) & IMX_UART_UTS_TXFULL) == 0) {
               if (interruptContextPtr->RxHoldState == IMX_UART_STATE::PAUSING) {
                   WRITE_REGISTER_NOFENCE_ULONG(
                       &registersPtr->Txd,
                       SERIAL_DEF_XOFF);
                   IMX_UART_LOG_TRACE("XOFF sent.");
                   interruptContextPtr->RxHoldState = IMX_UART_STATE::PAUSED;
               } else {
                   if (interruptContextPtr->RxHoldState == IMX_UART_STATE::RESUMING) {
                       WRITE_REGISTER_NOFENCE_ULONG(
                           &registersPtr->Txd,
                           SERIAL_DEF_XON);
                       IMX_UART_LOG_TRACE("XON sent.");
                       interruptContextPtr->RxHoldState = IMX_UART_STATE::STOPPED;
                   }
               }
           }
        }
        ...
        // Xon/Xoff: Sending XOFF means stop transmission.
        //
        if ((interruptContextPtr->TxHoldState != IMX_UART_STATE::PAUSED) &&
            interruptContextPtr->RxHoldState != IMX_UART_STATE::PAUSED) {
...
NTSTATUS IMXUartEvtSerCx2FileOpen (WDFDEVICE WdfDevice)
{
    ...
    NT_ASSERT(interruptContextPtr->TxHoldState == IMX_UART_STATE::STOPPED);
...
VOID IMXUartEvtSerCx2FileClose (WDFDEVICE WdfDevice)
{
    ...
    // In case Xon/Xoff is used at receiver side and XOFF was send,
    // unblock the device by sending XON prior to close.
    if (interruptContextPtr->CurrentFlowReplace & SERIAL_AUTO_RECEIVE) {
        if (interruptContextPtr->RxHoldState == IMX_UART_STATE::PAUSED) {
            // Disable Tx interrupt
            WRITE_REGISTER_NOFENCE_ULONG(&interruptContextPtr->RegistersPtr->Ucr1,
                                         interruptContextPtr->Ucr1Copy &
                                         ~(IMX_UART_UCR1_TRDYEN | IMX_UART_UCR1_TXMPTYEN));
            // Send Xon
            WRITE_REGISTER_NOFENCE_ULONG(&interruptContextPtr->RegistersPtr->Ucr1, SERIAL_DEF_XON);
            // Wait for Tx FIFO empty flag
            while (!(READ_REGISTER_NOFENCE_ULONG(&interruptContextPtr->RegistersPtr->Usr2) & IMX_UART_USR2_TXFE));
        }
    }
    ...
    interruptContextPtr->WaitEvents = 0;
    interruptContextPtr->CurrentFlowReplace = 0;
}
    ...
    )
{
    ...
    if ((interruptContextPtr->RxHoldState == IMX_UART_STATE::PAUSED) &&
        (interruptContextPtr->RxBuffer.Count() <= interruptContextPtr->XonThreshold)) {
        //
        // Enable TX interrupts to send XON
        //
        IMX_UART_LOG_TRACE("Enable TX interrupts to send XON");
        interruptContextPtr->RxHoldState = IMX_UART_STATE::RESUMING;
        IMX_UART_REGISTERS* registersPtr = interruptContextPtr->RegistersPtr;
        WdfInterruptAcquireLock(interruptContextPtr->WdfInterrupt);
        interruptContextPtr->Ucr1Copy |= (IMX_UART_UCR1_TRDYEN | IMX_UART_UCR1_TXMPTYEN);
        interruptContextPtr->Usr1EnabledInterruptsMask |= IMX_UART_USR1_TRDY;
        interruptContextPtr->Usr2EnabledInterruptsMask |= IMX_UART_USR2_TXFE;
        WRITE_REGISTER_NOFENCE_ULONG(
            &registersPtr->Ucr1,
...
static VOID
IMXUartEnableRxInterrupt(
    _Inout_ IMX_UART_INTERRUPT_CONTEXT* InterruptContextPtr
)
{
    ...
    )
{
    ...
    // Enable RRDY interrupt and aging character interrupt
    IMXUartEnableRxInterrupt(interruptContextPtr);
    ...
    )
{
    ...
    // Pause Tx due to received Xoff or Xoff sent
    if ((interruptContextPtr->TxHoldState == IMX_UART_STATE::PAUSED) ||
        (interruptContextPtr->RxHoldState == IMX_UART_STATE::PAUSED)) {
    ...
    )
{
    ...
    switch (interruptContextPtr->TxHoldState) {
    case !IMX_UART_STATE::STOPPED:
        return STATUS_INVALID_DEVICE_STATE;
    }

    switch (interruptContextPtr->RxHoldState) {
    case !IMX_UART_STATE::STOPPED:
        return STATUS_INVALID_DEVICE_STATE;
    }
    ...
    )
{
    ...
    if ((LineControlPtr->FlowReplace & ~(SERIAL_RTS_MASK | SERIAL_AUTO_TRANSMIT |
                                         SERIAL_AUTO_RECEIVE)) != 0) {
        IMX_UART_LOG_ERROR(
    ...
    if (LineControlPtr->FlowReplace & (SERIAL_AUTO_TRANSMIT | SERIAL_AUTO_RECEIVE)) {
        if (interruptContextPtr->RxDmaTransactionContextPtr != nullptr) {
            IMX_UART_LOG_ERROR("Xon/Xoff mode is supported only in interrupt mode.");
            return STATUS_NOT_SUPPORTED;
        }
        if (LineControlPtr->FlowReplace & SERIAL_AUTO_RECEIVE) {
            // XonLimit
            if ((LineControlPtr->XonLimit < 0) ||
                ((ULONG)LineControlPtr->XonLimit > DeviceContextPtr->Parameters.RxIntermediateBufferSize) ||
                ((ULONG)LineControlPtr->XonLimit > (ULONG)LineControlPtr->XoffLimit)) {
                return STATUS_INVALID_PARAMETER;
            }
            if (LineControlPtr->XonLimit != 0) {
                interruptContextPtr->XonThreshold = LineControlPtr->XonLimit;
            } else {
                interruptContextPtr->XonThreshold = DeviceContextPtr->Parameters.RxIntermediateBufferSize >> 3;
            }
            // XoffLimit
            if ((LineControlPtr->XoffLimit < 0) ||
                ((ULONG)LineControlPtr->XoffLimit > DeviceContextPtr->Parameters.RxIntermediateBufferSize)) {
                return STATUS_INVALID_PARAMETER;
            }
            if (LineControlPtr->XoffLimit != 0) {
                interruptContextPtr->XoffThreshold = LineControlPtr->XoffLimit;
            } else {
                interruptContextPtr->XoffThreshold = DeviceContextPtr->Parameters.RxIntermediateBufferSize >> 1;
            }
        }
        IMX_UART_LOG_TRACE("FlowReplace value: 0x%x", LineControlPtr->FlowReplace);
    }
    ...
    //
    {
        ...
        interruptContextPtr->CurrentFlowReplace = LineControlPtr->FlowReplace;
        interruptContextPtr->Ucr2Copy &= ~clearMask;
        interruptContextPtr->Ucr2Copy |= setMask;
        WRITE_REGISTER_NOFENCE_ULONG(
            &interruptContextPtr->RegistersPtr->Ucr2,
        ...
        DeviceContextPtr->CurrentFlowReplace = interruptContextPtr->CurrentFlowReplace;
        WdfInterruptReleaseLock(interruptContextPtr->WdfInterrupt);
    }
    ...
    )
{
    ...
    //
    // Nothing to do here for SERIAL_EV_ERR -
    ...
    )
{
    ...
    *outputBufferPtr = SERIAL_HANDFLOW();
    ...
    if (DeviceContextPtr->CurrentFlowReplace & (SERIAL_AUTO_TRANSMIT)) {
        outputBufferPtr->FlowReplace |= SERIAL_AUTO_TRANSMIT;
    }
    if (DeviceContextPtr->CurrentFlowReplace & (SERIAL_AUTO_RECEIVE)) {
        outputBufferPtr->FlowReplace |= SERIAL_AUTO_RECEIVE;
        outputBufferPtr->XonLimit = DeviceContextPtr->InterruptContextPtr->XonThreshold;
        outputBufferPtr->XoffLimit = DeviceContextPtr->InterruptContextPtr->XoffThreshold;
    }
    ...
    )
{
    ...
    *outputBufferPtr = SERIAL_STATUS();
    ...
    if (DeviceContextPtr->CurrentFlowReplace & SERIAL_AUTO_TRANSMIT) {
        if (interruptContextPtr->TxHoldState == IMX_UART_STATE::PAUSED) {
            outputBufferPtr->HoldReasons |= SERIAL_TX_WAITING_FOR_XON;
        }
    }
    if (DeviceContextPtr->CurrentFlowReplace & SERIAL_AUTO_RECEIVE) {
        // "Transmission is waiting because the XOFF character was transmitted."
        if (interruptContextPtr->RxHoldState == IMX_UART_STATE::PAUSED) {
            outputBufferPtr->HoldReasons |= SERIAL_TX_WAITING_XOFF_SENT;
        }
    }
    ...
    )
{
    ...
    case UART_SERIAL_FLAG_FLOW_CTL_XONXOFF:
        ...
        HandflowPtr->FlowReplace |= SERIAL_AUTO_TRANSMIT;
        HandflowPtr->FlowReplace |= SERIAL_AUTO_RECEIVE;
        break;
    ...
    )
{
    ...
    if (deviceContextPtr->Parameters.RxDmaMinTransactionLength == 0) {
        ...
        highestAcceptableAddress.QuadPart = (LONGLONG)MAXULONG;
