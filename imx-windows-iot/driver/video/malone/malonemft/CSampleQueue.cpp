    ...
    do {
...

        {
            ...
            if (m_bAddMarker != FALSE) {
                hr = pSample->SetUINT64(MaloneMft_MFSampleExtension_Marker, m_pulMarkerID);
                if (FAILED(hr)) {
                    break;
                }
            ...
            if (IsQueueEmpty() != FALSE) {
                ...
                TraceString(CHMFTTracing::TRACE_INFORMATION, L"%S(): Sample @%p is first sample in Queue @%p",  __FUNCTION__, pSample,
                            this);
            } else {
                m_pTail->pNext  = pNewNode;
                m_pTail         = pNewNode;

                TraceString(CHMFTTracing::TRACE_INFORMATION, L"%S(): Sample @%p added to back of Queue @%p",  __FUNCTION__, pSample,
                            this);
            }
            m_ulSize++;
        }
    ...
    do {
        ...
        *ppSample   = NULL;

        {
            ...
            TraceString(CHMFTTracing::TRACE_INFORMATION, L"%S(): Sample @%p removed from queue Queue @%p",  __FUNCTION__,
                        (*ppSample), this);
