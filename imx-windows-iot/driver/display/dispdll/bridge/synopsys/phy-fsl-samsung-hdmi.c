...
const struct phy_config samsung_phy_pll_cfg[] = {
	{	22250000, {
		...
		71000000, {
			0x00, 0xD1, 0x58, 0x58, 0x87, 0x06, 0x87, 0x46,
		...
		85500000, {
			0x00, 0xD1, 0x6B, 0x58, 0x87, 0x81, 0x9C, 0x45,
			0x4F, 0x30, 0x33, 0x65, 0x10, 0x90, 0x24, 0x80,
			0x6C, 0xF2, 0x67, 0x00, 0x10, 0x85, 0x30, 0x32,
			0x60, 0x8F, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00,
			0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0xE0, 0x83, 0x0F, 0x3E, 0xF8, 0x00, 0x00,
...
long samsung_hdmi_phy_clk_round_rate(unsigned long rate)
{
	...
	unsigned long best_rate = 0, best_diff = 297000000, diff;

	for (; phy_cfg->clk_rate != 0; phy_cfg++) {
		diff = abs(phy_cfg->clk_rate - rate);
		if (diff < best_diff) {
			best_diff = diff;
			best_rate = phy_cfg->clk_rate;
		}
	}
...
int samsung_hdmi_phy_clk_set_rate(struct platform_device *pdev,
					     unsigned long rate)
{
	struct samsung_hdmi_phy *samsung = platform_get_drvdata(pdev);
	const struct phy_config *phy_cfg = samsung_phy_pll_cfg;
	int i;
	...
	if (phy_cfg->clk_rate == 0) {
		dev_err(samsung->dev, "HDMI phy clk_rate not supported (%d)\n", rate);
		return -EINVAL;
	}
	dev_info(samsung->dev, "HDMI phy found rate (%d)\n", phy_cfg->clk_rate);
...
int samsung_hdmi_phy_probe(struct platform_device *pdev, u8 __iomem* hdmi_regs, u32 phy_offset)
{
	...
	if (hdmi_regs) {
		samsung->regs = hdmi_regs + phy_offset;
		samsung->regs_allocated = false;
	} else {
		res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
		regs = devm_ioremap_resource(samsung->dev, res);
		if (IS_ERR(regs))
			return PTR_ERR(regs);
		samsung->regs_allocated = true;
		samsung->regs = regs;
	}
...
int samsung_hdmi_phy_remove(struct platform_device *pdev)
{
	...
	if (samsung) {
		if (samsung->regs && samsung->regs_allocated) {
			res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
			if (res)
				iounmap(samsung->regs, resource_size(res));
		}
		devm_kfree(&pdev->dev, samsung);
	}
