				     ...
				     struct phy_configure_opts_mipi_dphy *cfg)
{
	...
	do_div(&ui, (unsigned int)hs_clk_rate);

	cfg->clk_miss = 0;
	cfg->clk_post = 60000 + 52 * (unsigned int)ui;
	cfg->clk_pre = 8;
	...
	cfg->eot = 0;
	cfg->hs_exit = 100000;
	cfg->hs_prepare = 40000 + 4 * (unsigned int)ui;
	cfg->hs_zero = 105000 + 6 * (unsigned int)ui;
	cfg->hs_settle = 85000 + 6 * (unsigned int)ui;
	cfg->hs_skip = 40000;
	 ...
	 */
	cfg->hs_trail = max(4 * 8 * (unsigned int)ui, 60000 + 4 * 4 * (unsigned int)ui);
	...
	cfg->hs_clk_rate = (unsigned long)hs_clk_rate;
	cfg->lanes = (unsigned char)lanes;
