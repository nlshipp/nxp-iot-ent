...
struct dprc {
	...
	struct _sc_ipc_struct_t *ipc_handle;
...
bool dprc_is_disabled(struct dprc *dprc)
{
	if (!dprc) {
		WARN_ON(1);
		return false;
	...
	if (dprc->has_aux_prg)
		return prg_is_disabled(dprc->prgs[1]);

	return prg_is_disabled(dprc->prgs[0]);
}
...
dprc_dpu_gpr_configure(struct dprc *dprc, unsigned int stream_id)
{
	...
	ret = sc_misc_set_control(dprc->ipc_handle,
		(u16)dprc->sc_resource, IMX_SC_C_KACHUNK_SEL, stream_id);
	if (ret)
...
dprc_prg_sel_configure(struct dprc *dprc, u32 resource, bool enable)
{
	...
	ret = sc_misc_set_control(dprc->ipc_handle,
		(u16)resource, IMX_SC_C_SEL0, enable);
	if (ret)
	...
	struct platform_device *prg_pdev, int prg_cnt)
{
	...
	dprc->ipc_handle = imx_scu_get_handle();
	if (!dprc->ipc_handle)
		return -EINVAL;
	...
	ret = of_property_read_u32_array(&pdev->dev.of_node,
		"fsl,prgs", &prgs[0], 2);
	if (ret) {
		dev_err(dev, "cannot get PRG ids %d\n", ret);
		return ret;
	}
	...
	for (i = 0; i < 2; i++) {
		...
		if (prgs[i] > (u32)prg_cnt) {
			dev_err(dev, "wrong PRG id %u\n", prgs[i]);
			return -EINVAL;
		}

		dprc->prgs[i] = platform_get_drvdata(&prg_pdev[prgs[i]]);
		if (!dprc->prgs[i])
			return -EPROBE_DEFER;
...
int dprc_remove(struct platform_device *pdev, bool stop)
{
	...
	if (res && dprc->base)
		devm_iounmap(&pdev->dev, dprc->base, resource_size(res));
