...
bool prg_is_disabled(struct prg *prg)
{
	if (!prg) {
		WARN_ON(1);
		return false;
	...
	return prg_read(prg, PRG_CTRL) & BYPASS;
}
...
int prg_probe(struct platform_device *pdev)
{
	...
	if (prg_is_disabled(prg))
		prg_reset(prg);

	return 0;
}

int prg_remove(struct platform_device *pdev, bool stop)
{
	...
	if (res && prg->base)
		devm_iounmap(&pdev->dev, prg->base, resource_size(res));
