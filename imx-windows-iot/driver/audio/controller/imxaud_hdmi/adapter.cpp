    ...
    )
{
    ...
    NTSTATUS            ntStatus;
    PENDPOINT_MINIPAIR* ppMiniportPair = g_HdmiEndpoints;
    
    for (ULONG i = 0; i < g_cHdmiEndpoints; ++i, ++ppMiniportPair)
    {
        ntStatus = InstallEndpointFilters(DeviceObject, Irp, AdapterCommon, *ppMiniportPair);
...
--*/
    ...
    ntStatus = pAdapterCommon->Init(DeviceObject, ResourceList);
    IF_FAILED_JUMP(ntStatus, Exit);
    ...
    ntStatus = ConnectHDMIAudio(DeviceObject, pAdapterCommon);
...
--*/
{
        ...
        (IRP_MN_STOP_DEVICE == stack->MinorFunction))
    {
        ...
        if (ext->m_pCommon != NULL)
        {
            ...
            ULONG i = g_cHdmiEndpoints;
            while (i--) 
            {
                ntStatus = ext->m_pCommon->RemoveEndpointFilters(g_HdmiEndpoints[i], NULL, NULL);
                NT_ASSERT(NT_SUCCESS(ntStatus));
            }
