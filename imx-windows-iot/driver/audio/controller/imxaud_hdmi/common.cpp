...
/* #define DEBUG_HDMI_DISP_INTERFACE */
#ifdef DEBUG_HDMI_DISP_INTERFACE
#define debug_printk_iface(x, ...) DbgPrintEx(DPFLTR_IHVAUDIO_ID, DPFLTR_ERROR_LEVEL, x, __VA_ARGS__)
#else
#define debug_printk_iface(x, ...)
#endif
    ...
    public CUnknown                              // stdunk.h
{
    private:
        ...
        BOOL                    m_pHdmiAudioInterfaceRegistered;
        USHORT                  m_pHdmiAudiointerfaceVersion; //version of the interface
        USHORT                  m_pHdmiAudioInterfaceSize;  //size of the interface
        CONST GUID*             m_pHdmiAudioInterfaceType;  //GUID of the interface
        struct Imx8mpHdmiAudioInterface* m_pHdmiAudioInterface; //pointer to the interface to hdmi display driver
        FILE_OBJECT* m_pMonitorFile;  //handle to MONITOR device interface
        EventCallback           m_HdmiConnectionStatusCallback;
        LONG                    m_HdmiConnectionStatus;
        DXGK_CHILD_CONTAINER_ID m_HdmiContainerId;
        UNICODE_STRING          m_SymbolicLinkName;
        WCHAR                   m_SymbolicLinkNameBuffer[512];
    ...
    public:
        ...
        {
            return (InterlockedOr(&m_HdmiConnectionStatus, 0) == 0) ? FALSE : TRUE;
        }
        ...
        )
        {
            ...
            memcpy(ContainerId, &m_HdmiContainerId.ContainerId, sizeof(GUID));
        }
        ...
        )
        {
            ...
            SinkInfo->ManufacturerId  = m_HdmiContainerId.EldInfo.ManufacturerName;
        ...
        friend NTSTATUS
            ConnectHDMIAudio(
                ...
                _In_ PADAPTERCOMMON  pAdapterCommon);

        NTSTATUS
            AdapterRegisterDevInterface(
                _In_ FILE_OBJECT* pMonitorFile,
                _In_ PUNICODE_STRING pSymbolicLinkName);

        VOID
            AdapterUnregisterDevInterface(
                VOID);

        NTSTATUS
            QueryHDMIAudioInterface(
                FILE_OBJECT*& pMonitorFile,
                PUNICODE_STRING SymbolicLinkName);

        friend NTSTATUS
            MonitorDevicceInterfaceCallback(
                DEVICE_INTERFACE_CHANGE_NOTIFICATION* pMonitorInfo,
                PVOID   Context);
        //=====================================================================
        // friends
...
--*/
{
    ...
    this->AdapterUnregisterDevInterface();

    if (m_NotificationEntry)
    {
        IoUnregisterPlugPlayNotificationEx(m_NotificationEntry);
    }
    if (m_pHdmiAudioInterface)
    {
        delete m_pHdmiAudioInterface;
        m_pHdmiAudioInterface = NULL;
    }
...
--*/
{
    ...
    m_pMonitorFile          = NULL;
    m_pHdmiAudioInterface   = NULL;
    m_pHdmiAudioInterfaceRegistered = FALSE;
    m_NotificationEntry     = NULL;
    InitializeListHead(&m_SubdeviceCache);
    RtlInitEmptyUnicodeString(&m_SymbolicLinkName, m_SymbolicLinkNameBuffer, sizeof(m_SymbolicLinkNameBuffer));
    m_HdmiConnectionStatus = FALSE;
    RtlZeroMemory(&m_HdmiConnectionStatusCallback, sizeof(m_HdmiConnectionStatusCallback));
    ...
    ntStatus = IoGetDeviceProperty(m_pPhysicalDeviceObject,
                                 DevicePropertyHardwareID,
                                 sizeof(DeviceId), &DeviceId, &ResultLen);
    IF_FAILED_ACTION_JUMP(ntStatus, DPF(D_ERROR, ("WdfDeviceMiniportCreate failed, 0x%x", ntStatus)), Done);

    if (!wcscmp(DeviceId, L"ACPI\\VEN_NXP&DEV_0130")) /* NXP0130 = HDMI audio for 8MP */
    {
        m_Soc = new(NonPagedPoolNx, MINADAPTER_POOLTAG) CSoc_htx();
        if (m_Soc == NULL)
        {
            ntStatus = STATUS_INSUFFICIENT_RESOURCES;
            DPF(D_ERROR, ("CSoc_htx failed, 0x%x", ntStatus));
            goto Done;
        }
        m_pHdmiAudioInterface = new(NonPagedPoolNx, MINADAPTER_POOLTAG) Imx8mpHdmiAudioInterface();
        if (m_pHdmiAudioInterface == NULL)
        {
            ntStatus = STATUS_INSUFFICIENT_RESOURCES;
            DPF(D_ERROR, ("Imx8mpHdmiAudioInterface failed, 0x%x", ntStatus));
            goto Done;
        }
        m_pHdmiAudioInterfaceType = &GUID_DEVINTERFACE_IMX8_PLUS_HDMI_AUDIO;
        m_pHdmiAudioInterfaceSize = sizeof(Imx8mpHdmiAudioInterface);
        m_pHdmiAudiointerfaceVersion = IMX8_PLUS_HDMI_AUDIO_INTERFACE_VERSION;

    }
    else {
        ntStatus = STATUS_NOT_SUPPORTED;
        DPF(D_ERROR, ("Device NOT supported, 0x%x", ntStatus));
        goto Done;
    }
...
{
    PAGED_CODE();
    return m_Soc->FreeBuffer(Stream, DeviceType, Mdl, Size);
}
...
{
    PAGED_CODE();
    return m_Soc->AllocBuffer(Stream, DeviceType, Size, pMdl, CacheType);
}
...
)
{
    ...
    Status = QueryHDMIAudioInterface(pMonitorFile, &m_SymbolicLinkName);
    if (!NT_SUCCESS(Status))
    {
        debug_printk_iface("CAdapterCommon::RegisterStream: QuerryHDMIAudioInterface: ERROR (%S).\n", m_SymbolicLinkName.Buffer);
        return Status;
    }

    AdapterRegisterDevInterface(pMonitorFile, &m_SymbolicLinkName);
    debug_printk_iface("CAdapterCommon::RegisterStream: QuerryHDMIAudioInterface: OK (%S).\n", m_SymbolicLinkName.Buffer);

    PWAVEFORMATEXTENSIBLE  pWfExt = Stream->GetDataFormat();
    struct imx8mp_hdmi_audio_params audio_params;
    audio_params.channels = pWfExt->Format.nChannels;
    audio_params.sample_rate = pWfExt->Format.nSamplesPerSec;
    audio_params.sample_width = pWfExt->Samples.wValidBitsPerSample;
    audio_params.iec958_aes0_nonaudio = 0;
    (*m_pHdmiAudioInterface->AudioSetHwParams)(m_pHdmiAudioInterface->Context, &audio_params);
...
)
{
    ...
    PAGED_CODE();
    Status = m_Soc->UnregisterStream(Stream, DeviceType);
    AdapterUnregisterDevInterface();
    debug_printk_iface("CAdapterCommon::UnregisterStream: OK (%S).\n", m_SymbolicLinkName.Buffer);
    return Status;
}
...
{
    PAGED_CODE();
    if (!m_pHdmiAudioInterfaceRegistered)
    {
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    (*m_pHdmiAudioInterface->AudioMuteStream)(m_pHdmiAudioInterface->Context, FALSE);
...
)
{
    ...
    if (m_pHdmiAudioInterfaceRegistered)
    {
        (*m_pHdmiAudioInterface->AudioMuteStream)(m_pHdmiAudioInterface->Context, TRUE);
    }
...
)
{
    ...
    m_HdmiConnectionStatusCallback.Handler = EventHandler; // weak ref.
    m_HdmiConnectionStatusCallback.Context = EventHandlerContext;
}
    ...
    _In_ PADAPTERCOMMON  pAdapterCommon)
{
    ...
    NTSTATUS    Status;
    CAdapterCommon* pAdapter = (CAdapterCommon*)pAdapterCommon;
    ...
    Status = IoRegisterPlugPlayNotification(
        ...
        (PDRIVER_NOTIFICATION_CALLBACK_ROUTINE)MonitorDevicceInterfaceCallback,
        pAdapter,
        &pAdapter->m_NotificationEntry);
    if (!NT_SUCCESS(Status))
    {
        return Status;
    }
...
#pragma code_seg()
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::AdapterRegisterDevInterface(
    _In_ FILE_OBJECT* pMonitorFile,
    _In_ PUNICODE_STRING pSymbolicLinkName)
{
    NTSTATUS    Status = STATUS_INSUFFICIENT_RESOURCES;

    if (pSymbolicLinkName)
    {
        m_pMonitorFile = pMonitorFile;
        m_pHdmiAudioInterfaceRegistered = TRUE;
        Status = RtlUnicodeStringCopy(&m_SymbolicLinkName, pSymbolicLinkName);
    }
    return Status;
}
...
#pragma code_seg()
VOID
CAdapterCommon::AdapterUnregisterDevInterface(VOID)
{
    if (m_pHdmiAudioInterfaceRegistered)
    {
        (*m_pHdmiAudioInterface->InterfaceDereference)(m_pHdmiAudioInterface->Context);
        m_pHdmiAudioInterfaceRegistered = FALSE;
    }
    if (m_pMonitorFile)
    {
        ObDereferenceObject(m_pMonitorFile);
        m_pMonitorFile = NULL;
    }
}
...
#pragma code_seg()
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::QueryHDMIAudioInterface(
    FILE_OBJECT*& pMonitorFile,
    PUNICODE_STRING SymbolicLinkName)
{
    KEVENT  Event;
    ...
    if (!pIrp)
    {
        ObDereferenceObject(pMonitorFile);
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    pIrpSp->Parameters.QueryInterface.InterfaceType = m_pHdmiAudioInterfaceType;
    pIrpSp->Parameters.QueryInterface.Size = m_pHdmiAudioInterfaceSize;
    pIrpSp->Parameters.QueryInterface.Version = m_pHdmiAudiointerfaceVersion;
    pIrpSp->Parameters.QueryInterface.Interface = (PINTERFACE)m_pHdmiAudioInterface;

    Status = IoCallDriver(pMonitorDevice, pIrp);
    ...
    PVOID   pContext)
{
    NTSTATUS    Status = STATUS_SUCCESS;
    FILE_OBJECT* pMonitorFile;
    CAdapterCommon* pAdapter = (CAdapterCommon*)pContext;
    ...
    if (RtlEqualMemory(
        ...
        sizeof(GUID)))
    {
        /* Try query DEVINTERFACE_*_HDMI_AUDIO */
        Status = pAdapter->QueryHDMIAudioInterface(pMonitorFile, pMonitorInfo->SymbolicLinkName);
        if (!NT_SUCCESS(Status))
        {
            /* This display instance doesn't support audio-hdmi device interface */
            debug_printk_iface("DEVICE_INTERFACE_ARRIVAL: QuerryHDMIAudioInterface: ERROR (%S).\n", pMonitorInfo->SymbolicLinkName->Buffer);
            return Status;
        }

        pAdapter->m_pHdmiAudioInterface->AudioGetContainerId(pAdapter->m_pHdmiAudioInterface->Context, &pAdapter->m_HdmiContainerId);

        /* Release interface immediately in order to not block display driver restart - interface will be queried again during new stream registering */
        (*pAdapter->m_pHdmiAudioInterface->InterfaceDereference)(pAdapter->m_pHdmiAudioInterface->Context);
        ObDereferenceObject(pMonitorFile);

        (VOID)InterlockedExchange(&pAdapter->m_HdmiConnectionStatus, TRUE);
        /* Display is hdmi audio capable - store Symlink name */
        Status = RtlUnicodeStringCopy(&pAdapter->m_SymbolicLinkName, pMonitorInfo->SymbolicLinkName);
        if (!NT_SUCCESS(Status))
        {
            debug_printk_iface("DEVICE_INTERFACE_ARRIVAL: Copy SymbolicLinkName ERROR.\n");
            return Status;
        }

        /* Notify Jack presence */
        debug_printk_iface("DEVICE_INTERFACE_ARRIVAL: QuerryHDMIAudioInterface: OK. Jack connected (%S)\n", pMonitorInfo->SymbolicLinkName->Buffer);
        if (pAdapter->m_HdmiConnectionStatusCallback.Handler != NULL)
        {
            pAdapter->m_HdmiConnectionStatusCallback.Handler(
                pAdapter->m_HdmiConnectionStatusCallback.Context);
        }
    }
    ...
    if (RtlEqualMemory(
        ...
        sizeof(GUID)))
    {
        if (pMonitorInfo->SymbolicLinkName == NULL)
        {
            debug_printk_iface("DEVICE_INTERFACE_REMOVAL: SymLink NULL\n");
            return STATUS_INVALID_PARAMETER;
        }
        if (!RtlEqualUnicodeString(pMonitorInfo->SymbolicLinkName, &pAdapter->m_SymbolicLinkName, TRUE))
        {
            /* This display instance was not registered previously during DEVICE_INTERFACE_ARRIVAL */
            debug_printk_iface("DEVICE_INTERFACE_REMOVAL: SymLink match: ERROR.(%S)\n", pMonitorInfo->SymbolicLinkName->Buffer);
            return STATUS_NOT_SUPPORTED;
        }
        /* The same display with hdmi audio capability as registered */
        debug_printk_iface("DEVICE_INTERFACE_REMOVAL: SymLink match: OK. Jack disconnected. (%S)\n", pMonitorInfo->SymbolicLinkName->Buffer);

        /* Notify Jack removal */
        (VOID)InterlockedExchange(&pAdapter->m_HdmiConnectionStatus, FALSE);
        if (pAdapter->m_HdmiConnectionStatusCallback.Handler != NULL)
        {
            pAdapter->m_HdmiConnectionStatusCallback.Handler(
                pAdapter->m_HdmiConnectionStatusCallback.Context);
