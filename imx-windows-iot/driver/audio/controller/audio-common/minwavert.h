...
private:
    ...
    // weak ref of running streams. Only one stream is supported.
    PCMiniportWaveRTStream              m_pStream;
...
#pragma code_seg("PAGE")
    ...
    BOOL IsStreamingPin(ULONG nPinId)
    {
        PAGED_CODE();

        PINTYPE pinType = m_DeviceFormatsAndModes[nPinId].PinType;
        return ((pinType == SystemCapturePin) || (pinType == SystemRenderPin));
    }
