...
{
    NTSTATUS            ntStatus;
    PENDPOINT_MINIPAIR* ppMiniportPair     = g_CaptureEndpoints;
    
    PAGED_CODE();

    for(ULONG i = 0; i < g_cCaptureEndpoints; ++i, ++ppMiniportPair)
    {
        ntStatus = InstallEndpointFilters(_pDeviceObject, _pIrp, _pAdapterCommon, *ppMiniportPair);
        IF_FAILED_JUMP(ntStatus, Exit);
    }
