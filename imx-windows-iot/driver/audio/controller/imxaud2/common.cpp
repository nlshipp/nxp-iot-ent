    ...
    public CUnknown                              // stdunk.h
{
    ...
    public:
        ...
        {
            return m_Soc.isRenderSupported();
        }
        STDMETHODIMP_(BOOL) isCaptureSupported(VOID)
        {
            return m_Soc.isCaptureSupported();
        }
        //=====================================================================
        // friends
...
--*/
{
    ...
    PCM_PARTIAL_RESOURCE_DESCRIPTOR rxDmaResourcePtr = nullptr;
    ...
    rxDmaResourcePtr = ResourceList->FindTranslatedEntry(CmResourceTypeDma, 0);
    ASSERT(interruptDescriptor);

    if (rxDmaResourcePtr == NULL)
    {
        ntStatus = STATUS_INSUFFICIENT_RESOURCES;
        goto Done;
    }

    txDmaResourcePtr = ResourceList->FindTranslatedEntry(CmResourceTypeDma, 1);
    ...
    for (index = 0; index < ResourceList->NumberOfEntries(); index++)
    {
		...
		{
			ntStatus = m_Soc.InitSaiBlock(registersDescriptor, interruptDescriptor, rxDmaResourcePtr, txDmaResourcePtr, m_pPhysicalDeviceObject);
		}
	}
