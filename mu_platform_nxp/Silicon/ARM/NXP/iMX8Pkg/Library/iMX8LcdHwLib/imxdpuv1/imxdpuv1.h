...
(((uint32_t)(__idx)<<0)|((uint32_t)(__eco_id)<<8)|((uint32_t)(__sub)<<16)|((uint32_t)(__blk_id)<<24))

#define get_channel_blk(chan) (((uint32_t)(chan) >> 24) & 0xff)
#define get_channel_sub(chan) (((uint32_t)(chan) >> 16) & 0xff)
#define get_eco_idx(chan)     (((uint32_t)(chan) >> 8) & 0xff)
#define get_channel_idx(chan) (((uint32_t)(chan) >> 0) & 0xff)
...
enum imxdpuv1_mode_flags {
	...
	IMXDPUV1_MODE_FLAGS_DISABLE_DE     = 1 << 20, /* Disables generation of the Data Enable signal by the frame generator. */
