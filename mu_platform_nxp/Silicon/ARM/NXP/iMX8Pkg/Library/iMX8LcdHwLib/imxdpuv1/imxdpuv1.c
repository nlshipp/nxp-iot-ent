...
{
	uint32_t val = 0;
	val = MmioRead32(imxdpu->base + offset);
	IMXDPUV1_TRACE_REG("R reg 0x%08x --> val 0x%08x\n",
		(uint32_t)imxdpu->base + offset, (uint32_t)val);
	return val;
}
...
void _imxdpuv1_write(struct imxdpuv1_soc *imxdpu, uint32_t offset, uint32_t value,
	char *file, int line)
{
	MmioWrite32(imxdpu->base + offset, value);
	IMXDPUV1_TRACE_REG("W reg 0x%08x <-- val 0x%08x\n",
		(uint32_t)imxdpu->base + offset, (uint32_t)value);
}
	...
	void *values, uint32_t cnt, char *file, int line)
{
	...
	for (i = 0; i < cnt; i++) {
		MmioWrite32(imxdpu->base + offset + (i*4), src[i]);
		IMXDPUV1_TRACE_REG("WB reg 0x%08x <-- val 0x%08x\n",
		imxdpu->base + offset + (i*4), (uint32_t)(src[i]));
	}
}
...
int imxdpuv1_enable_irq(int8_t imxdpuv1_id, uint32_t irq)
{
	...
	if (irq == 0)
		DEBUG((DEBUG_ERROR, "Trying to enable irq 0!\n"));
#endif
	/* imxdpuv1_id starts from 0 */
	if (!((imxdpuv1_id >= 0) && (imxdpuv1_id < IMXDPUV1_MAX_NUM))) {
		return -EINVAL;
...
int imxdpuv1_handle_irq(int32_t imxdpuv1_id)
{
...
#ifdef DEBUG_IMXDPUV1_IRQ_ERROR
		DEBUG((DEBUG_ERROR, "wrong imxdpuv1_id\n"));
#endif
		return IMXDPUV1_FALSE;
	}
	imxdpu = &imxdpuv1_array[imxdpuv1_id];
...
#ifdef DEBUG_IMXDPUV1_IRQ_ERROR
	{
		...
		int_enable0 = imxdpuv1_read_irq(imxdpu,
			IMXDPUV1_COMCTRL_USERINTERRUPTENABLE0);
		if (int_enable0 & 1) {
			DEBUG((DEBUG_ERROR, "IRQ0 enabled\n"));
		}
		if (imxdpu->enabled_int[0] & 1) {
			DEBUG((DEBUG_ERROR, "IRQ0 in enabled_int is set\n"));
		}
	}
#endif
...
#ifdef DEBUG_IMXDPUV1_IRQ_ERROR
		DEBUG((DEBUG_ERROR, "no enabled IMXDPU interrupts\n"));
#endif
...
/*!
 * This function initializes the required data structures
 *
 * @param	imxdpuv1_id	id of the diplay unit
 *
 * @return      This function returns 0 on success or negative error code on
 *      	fail.
 */
int imxdpuv1_init_drv(int8_t imxdpuv1_id)
{
	int i;
	struct imxdpuv1_soc *imxdpu;
	...
	if (imxdpuv1_id == 0) {
		imxdpu->base = IMXDPUV1_REGS_BASE_PHY0;
		IMXDPUV1_TRACE("%s(): virtual base address is 0x%x (0x%08x physical)\n",
			__func__, imxdpu->base, IMXDPUV1_REGS_BASE_PHY0);

	} else if (imxdpuv1_id == 1) {
		imxdpu->base = IMXDPUV1_REGS_BASE_PHY1;
		IMXDPUV1_TRACE("%s(): virtual base address is 0x%x (0x%08x physical)\n",
...
int imxdpuv1_init(int8_t imxdpuv1_id)
{
	...
	imxdpuv1_write(imxdpu, IMXDPUV1_FETCHECO2_STATICCONTROL,
		IMXDPUV1_FETCHECO2_STATICCONTROL_RESET_VALUE);
	...
	const struct imxdpuv1_videomode *mode)
{
	...
	imxdpuv1_write(imxdpu, b_off + IMXDPUV1_TCON0_TCON_CTRL_OFFSET,
		...
		IMXDPUV1_SET_FIELD(IMXDPUV1_TCON0_TCON_CTRL_SPLITPOSITION, 0x140) |
		IMXDPUV1_TCON0_TCON_CTRL_BYPASS__BYPASS_MODE); /* Bypass => Enable TCON mode later */
	/* setup hsync */
	...
	if (mode->flags & IMXDPUV1_MODE_FLAGS_HSYNC_POL) {
		imxdpuv1_write(imxdpu, b_off + IMXDPUV1_TCON0_SMX0FCTTABLE_OFFSET, 2);
	} else {
	...
	imxdpuv1_write(imxdpu, b_off + IMXDPUV1_TCON0_SPG1POSON_OFFSET,
		IMXDPUV1_SET_FIELD(IMXDPUV1_TCON0_SPG1POSON_SPGPSON_X1, mode->hlen + mode->hfp) |
	...
	imxdpuv1_write(imxdpu, b_off + IMXDPUV1_TCON0_SPG1POSOFF_OFFSET,
		IMXDPUV1_SET_FIELD(IMXDPUV1_TCON0_SPG1POSOFF_SPGPSOFF_X1, mode->hlen + mode->hfp)|
	...
	if (mode->flags & IMXDPUV1_MODE_FLAGS_VSYNC_POL) {
		imxdpuv1_write(imxdpu, b_off + IMXDPUV1_TCON0_SMX1FCTTABLE_OFFSET, 2);
	} else {
	...
	if (mode->flags & IMXDPUV1_MODE_FLAGS_DISABLE_DE) {
		imxdpuv1_write(imxdpu, b_off + IMXDPUV1_TCON0_SMX2FCTTABLE_OFFSET, 0);  /* The DE signal will never be active */
	} else {
		/* Setup generator to generate the DE signal */
		if (mode->flags & IMXDPUV1_MODE_FLAGS_DE_POL) {
			imxdpuv1_write(imxdpu, b_off + IMXDPUV1_TCON0_SMX2FCTTABLE_OFFSET, 8);
		} else {
			imxdpuv1_write(imxdpu, b_off + IMXDPUV1_TCON0_SMX2FCTTABLE_OFFSET, 7);
		}
	}

	/* shadow load trigger (aka kachunk) */
	imxdpuv1_write(imxdpu, b_off + IMXDPUV1_TCON0_SPG4POSON_OFFSET,
		IMXDPUV1_SET_FIELD(IMXDPUV1_TCON0_SPG4POSON_SPGPSON_X4, 0) |
		IMXDPUV1_SET_FIELD(IMXDPUV1_TCON0_SPG4POSON_SPGPSON_Y4, mode->vlen + 1));
	...
	imxdpuv1_write(imxdpu, b_off + IMXDPUV1_TCON0_SPG4POSOFF_OFFSET,
		IMXDPUV1_SET_FIELD(IMXDPUV1_TCON0_SPG4POSOFF_SPGPSOFF_X4, 32) |
		IMXDPUV1_SET_FIELD(IMXDPUV1_TCON0_SPG4POSOFF_SPGPSOFF_Y4, mode->vlen + 1));
...
int imxdpuv1_disp_setup_tcon_operation_mode(
	int8_t imxdpuv1_id,
	int8_t disp)
{
	struct imxdpuv1_soc *imxdpu;
	...
	val = imxdpuv1_read(imxdpu, b_off + IMXDPUV1_TCON0_TCON_CTRL_OFFSET);
	val &= ~IMXDPUV1_TCON0_TCON_CTRL_BYPASS__BYPASS_MODE;
	imxdpuv1_write(imxdpu, b_off + IMXDPUV1_TCON0_TCON_CTRL_OFFSET, val);

	return 0;
}

int imxdpuv1_disp_wait_framegen_done(int8_t imxdpuv1_id, int8_t disp)
{
	int i = 0;
	...
	do {
		reg = imxdpuv1_read_irq(imxdpu, b_off + IMXDPUV1_FRAMEGEN0_FGENSTS_OFFSET);
		MicroSecondDelay(100);
		i++;
	} while((reg & IMXDPUV1_FRAMEGEN0_FGENSTS_ENSTS_MASK) && (i < 2000));

	if (reg & IMXDPUV1_FRAMEGEN0_FGENSTS_ENSTS_MASK) {
		DEBUG((DEBUG_ERROR, "imxdpuv1_disp_wait_framegen_done timeout %d ms! (disp = %d)\n", i/10, disp));
		return 1;
	}
...
int imxdpuv1_disp_framegen_wait_frm_cntr_move(int8_t imxdpuv1_id, int8_t disp)
{
	uint64_t time_ns;
	uint32_t b_off;     /* block offset for frame generator */
	uint32_t frame_index, last_frame_index;
	struct imxdpuv1_soc *imxdpu;
	...
	frame_index = imxdpuv1_read(imxdpu, b_off + IMXDPUV1_FRAMEGEN1_FGTIMESTAMP_OFFSET)
		 >> IMXDPUV1_FRAMEGEN1_FGTIMESTAMP_FRAMEINDEX_SHIFT;
	...
	do {
		...
		if (time > start_time) {
			time_ns = GetTimeInNanoSecond(time - start_time);
		} else {
			time_ns = GetTimeInNanoSecond(start_time - time);
		}
	} while ((last_frame_index == frame_index) && (time_ns < 100000000u));

	if (last_frame_index == frame_index) {
		DEBUG((DEBUG_ERROR, "imxdpuv1_disp_framegen_wait_frm_cntr_move timeout %d us! (disp = %d, cntr = %d, regadr = 0x%x rawval=0x%x)\n",
			time_ns/1000, disp, frame_index, b_off + IMXDPUV1_FRAMEGEN1_FGTIMESTAMP_OFFSET,
			imxdpuv1_read(imxdpu, b_off + IMXDPUV1_FRAMEGEN1_FGTIMESTAMP_OFFSET)));
	}
...
uint32_t imxdpuv1_get_colorcomponentbits(uint32_t fmt)
{
	...
	case IMXDPUV1_PIX_FMT_RGB565:
		return IMXDPUV1_SET_FIELD(IMXDPUV1_COLOR_BITSRED0, 5) |
			IMXDPUV1_SET_FIELD(IMXDPUV1_COLOR_BITSGREEN0, 6) |
			IMXDPUV1_SET_FIELD(IMXDPUV1_COLOR_BITSBLUE0, 5) |
...
uint32_t imxdpuv1_get_colorcomponentshift(uint32_t fmt)
{
	...
	case IMXDPUV1_PIX_FMT_RGB565:
		return IMXDPUV1_SET_FIELD(IMXDPUV1_COLOR_SHIFTRED0, 11) |
			IMXDPUV1_SET_FIELD(IMXDPUV1_COLOR_SHIFTGREEN0, 5) |
			IMXDPUV1_SET_FIELD(IMXDPUV1_COLOR_SHIFTBLUE0, 0) |
