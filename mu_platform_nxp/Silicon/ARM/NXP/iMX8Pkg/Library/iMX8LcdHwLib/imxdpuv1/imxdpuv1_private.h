...
uint32_t _imxdpuv1_read(struct imxdpuv1_soc *dpu, uint32_t offset, char *file, int line);
...
{
	return MmioRead32(dpu->base + offset);
}
#endif

#ifdef ENABLE_IMXDPUV1_TRACE_IRQ_READ
uint32_t _imxdpuv1_read_irq(struct imxdpuv1_soc *dpu, uint32_t offset, char *file, int line);
...
{
	MmioWrite32(dpu->base + offset, value);
}
#endif
