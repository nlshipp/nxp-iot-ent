...
/* Storage of the Framebuffer address */
static EFI_PHYSICAL_ADDRESS  FrameBaseAddressStorage = 0;
  ...
  )
{
  ...
  if (converter == transmitterUnknown) {
    do {
      ...
      if (status == EFI_SUCCESS) {
        converter = ADV7535;
        if ((displayInterface != imxMipiDsi) && (displayInterface != imxMipiDsi1)) {
            /* dsi is already requested, don't change it, so we can test DSI1 interface.
               Otherwise select DSI0 */
            displayInterface = imxMipiDsi;
        }
        DEBUG((DEBUG_ERROR, "ADV7535 probe SUCCEDED. Mipi-dsi%d display interface selected.\n",
            (displayInterface == imxMipiDsi1) ? 1 : 0));
  ...
  if (converter == transmitterUnknown) {
    if ((displayInterface == imxMipiDsi) || (displayInterface == imxMipiDsi1)) {
      ...
      DEBUG((DEBUG_ERROR, "Mipi-dsi%d display interface. Default resolution used. %dx%d pclk=%d Hz\n", 
            (displayInterface == imxMipiDsi1) ? 1 : 0, PreferredTiming.HActive, PreferredTiming.VActive,
  ...
  if (FixedPcdGet32(PcdDisplayReadEDID) == TRUE) {
    ...
    for (i = 0; i < BASIC_EDID_STRUCTURE_LENGTH; i++) {
      edid[i] = 0;
    }
...
#if LCDHWLIB_DEBUG_LEVEL == DEBUG_ERROR
    DEBUG((LCDHWLIB_DEBUG_LEVEL, "Full EDID:\n"));
    for (i = 0; i < BASIC_EDID_STRUCTURE_LENGTH; i++) {
      DEBUG((LCDHWLIB_DEBUG_LEVEL, "0x%02x,", edid[i]));
      if ((i+1) % 8 == 0)   DEBUG((LCDHWLIB_DEBUG_LEVEL, "\n"));
    }
#endif
...
{
  FrameBaseAddressStorage = FrameBaseAddress;

  DEBUG((LCDHWLIB_DEBUG_LEVEL, "FB addr: 0x%llx\n", FrameBaseAddressStorage));

  CHECK_STATUS_RETURN_ERR(DcPixelLinkEnDi(displayInterface, FALSE), "PixelLink disable");
  CHECK_STATUS_RETURN_ERR(DcVideoStop(displayInterface), "DC video stop");

  /* Call ldb stop for both MIPI-DSI/LVDS */
  CHECK_STATUS_RETURN_ERR(Imx8xLdbStop(displayInterface), "LDB stop");

  if ((displayInterface == imxMipiDsi) || (displayInterface == imxMipiDsi1)) {
  /*--------------------------MIPI----------------------------------------------*/
    /* Timing not needed for shutdown - pass NULL */
    CHECK_STATUS_RETURN_ERR(mipi_dsi_northwest_init(NULL, 4, DSI_FMT_RGB888, displayInterface), "MIPI DSI dummy init");
    CHECK_STATUS_RETURN_ERR(mipi_dsi_northwest_disable(), "MIPI DSI disable");
  }
  CHECK_STATUS_RETURN_ERR(DcPixelLinkStop(displayInterface), "PixelLink stop");
  CHECK_STATUS_RETURN_ERR(DcClockStop(displayInterface), "DC clock stop");
  ...
  )
{
  ...
  if (FrameBaseAddressStorage == 0){
    return EFI_INVALID_PARAMETER;
  }
  
  CHECK_STATUS_RETURN_ERR(DcSocInit(displayInterface, Timing->PixelClock), "DPU SoC config");

  if ((displayInterface == imxMipiDsi) || (displayInterface == imxMipiDsi1)) {
  /*--------------------------MIPI----------------------------------------------*/
    /* Mipi DSI set timing mode */
    CHECK_STATUS_RETURN_ERR(mipi_dsi_northwest_init(Timing, 4, DSI_FMT_RGB888, displayInterface), "MIPI DSI init");
    CHECK_STATUS_RETURN_ERR(mipi_dsi_northwest_host_attach(), "MIPI DSI config");
    if (converter == ADV7535) {
  ...
  /*--------------------------LVDS----------------------------------------------*/
    ...
    CHECK_STATUS_RETURN_ERR(Imx8xLdbSocSetup(displayInterface, Timing->PixelClock), "LDB SoC config");
    Imx8xLdbConfigure(displayInterface);
    if (converter == IT6263) {
  ...
  )
{
  ...
  (void)DcPixelLinkEnDi(displayInterface, FALSE);
  (void)DcVideoStop(displayInterface);

  (void)DcPixelLinkStop(displayInterface);

  if ((displayInterface == imxMipiDsi) || (displayInterface == imxMipiDsi1)) {
    (void)mipi_dsi_northwest_disable();
  /*--------------------------MIPI----------------------------------------------*/
  } else if ((displayInterface == imxLvds0) || (displayInterface == imxLvds1) || (displayInterface == imxLvds0dual)) {
  /*--------------------------LVDS----------------------------------------------*/
    (void)Imx8xLdbStop(displayInterface);
  } else {
    DEBUG ((DEBUG_ERROR, "Unsupported display interface %d\n", (int)displayInterface));
    return;
  }

  (void)DcClockStop(displayInterface);

  DEBUG ((DEBUG_ERROR, "LCD display interface %d shutdown\n",
    ((displayInterface == imxMipiDsi1) || (displayInterface == imxLvds1)) ? 1 : 0));
