/** @file
*
*  Compulab iMX8M Nano USB OTG and EHCI Controllers
*
*  CL-SOM-iMX8M Nano USB port 1 (i.MX8 Controller Core 0) supports for OTG signaling,
...
Device (USB1)
{
  ...
  Name (_CRS, ResourceTemplate () {
    MEMORY32FIXED (ReadWrite, 0x4c100100, 0x100, )
    Interrupt (ResourceConsumer, Level, ActiveHigh, SharedAndWake) { 219 }
  })

  OperationRegion (OTGM, SystemMemory, 0x4c100100, 0x100)
...
Device (USB2)
{
  ...
  Name (_CRS, ResourceTemplate () {
    MEMORY32FIXED (ReadWrite, 0x4c200100, 0x100, )
    Interrupt (ResourceConsumer, Level, ActiveHigh, SharedAndWake) { 220 }
  })

  OperationRegion (OTGM, SystemMemory, 0x4c200100, 0x100)
  Field (OTGM, WordAcc, NoLock, Preserve) {
    Offset (0x84),  // skip to register 84h
    PTSC, 32,       // port status control
    Offset (0xA8),  // skip to register A8h
